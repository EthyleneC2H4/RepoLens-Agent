...
class RepoLensConfig(BaseModel):
    ...
    subagent_max_steps: int = Field(default=6, ge=1, le=15)
    token_budget: int = Field(default=24_000, ge=256, le=1_000_000)
    reasoning_effort: Literal["none", "low", "medium", "high"] = "none"
    context_window: int = Field(default=16_000, ge=1_024)
    tool_output_max_lines: int = Field(default=120, ge=10, le=2_000)
    ...
    @field_validator("resume_session")
    @classmethod
    def validate_resume_session(cls, value: Path | None) -> Path | None:
        if value is None:
            return None
        path = value.expanduser().resolve()
        if not path.is_file():
            raise ValueError(f"resume session does not exist: {path}")
        return path
